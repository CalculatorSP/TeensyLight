...
void ScreenVideoCapture::captureHwnd(HWND window, cv::Rect2d targetArea, cv::Mat& dest)
{
    ...
    dest.create((int)targetArea.height, (int)targetArea.width, CV_8UC4);

    // Initialize a bitmap
    hbwindow = CreateCompatibleBitmap(hwindowDC, (int)targetArea.width, (int)targetArea.height);
    bi.biSize = sizeof(BITMAPINFOHEADER);
    bi.biWidth = (LONG)targetArea.width;
    // The negative height is required -- removing the inversion will make the image appear upside-down.
    bi.biHeight = -(LONG)targetArea.height;
    bi.biPlanes = 1;
    bi.biBitCount = 32;
    ...
    BitBlt(hwindowCompatibleDC, 0, 0, (int)targetArea.width, (int)targetArea.height, hwindowDC, (int)targetArea.x, (int)targetArea.y, SRCCOPY);
    // Copy into our own buffer as device-independent bitmap
    GetDIBits(hwindowCompatibleDC, hbwindow, 0, (UINT)targetArea.height, dest.data, (BITMAPINFO *)&bi, DIB_RGB_COLORS);
